...
import os
import shutil
if(path.exists(os.getcwd()+"\Coord\\")):
    folder = os.getcwd()+"\Coord\\"
...
def randomlyPlaceObject(node):
    ...
    h=open(os.getcwd()+'\Coord\Pos'+str(R.r)+'.txt', 'w+')
    h.write(str(newPosition))
    h.close
    h=open(os.getcwd()+'\Coord\Rot'+str(R.r)+'.txt', 'w+')
    h.write(str(newRotation))
    h.close
    R.r+=1
